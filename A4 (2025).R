...
Classificacao <- read_excel("DE_BIN_(2025).xlsx")
View(Classificacao)
...
dados$Science.Communication <- as.factor(dados$Science.Communication)
...
trainIndex <- createDataPartition(dados$Education, p = 0.7, list = FALSE)
...
modelo_tree <- rpart(Climate.Change ~ Ocean.Literacy + Climate.Literacy + Education +
                       Marine.Education + Environmental.Education +
                       Climate.Change.Education +  Citizen.Science + 
                       Global.Warming + Science.Communication, 
